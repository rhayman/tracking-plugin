...
typedef enum { uniform, gauss, ttl } stim_mode;
...
//	This helper class is an OSC server running its own thread to keep data
// transmission 	continuous.

class TrackingNode;

class TrackingServer : public osc::OscPacketListener, public Thread {
public:
  TrackingServer();
...
public:
  TrackingModule() {}
  TrackingModule(String name, int port, String address, String color,
...
public:
  ...
  EventChannel *posEventChannelPtr;
  TTLEventPtr turnoffEvent; // holds a turnoff event that must be added in a
                            // later buffer
};
...
public:
  ...
  /** If the processor has a custom editor, this method must be defined to
   * instantiate it. */
  ...
  bool addSource(String name, int port = 0, String address = "",
                 String color = "");
  ...
  /** Called every time the settings of an upstream plugin are changed.
          Allows the processor to handle variations in the channel configuration
     or any other parameter passed through signal chain. The processor can use
     this function to modify channel objects that will be passed to downstream
     plugins. */
  void updateSettings() override;

  /** Defines the functionality of the processor.
          The process method is called every time a new data buffer is
     available. Visualizer plugins typically use this method to send data to the
     canvas for display purposes */
  ...
  StringArray colors = {"red",    "green", "blue", "magenta", "cyan",
